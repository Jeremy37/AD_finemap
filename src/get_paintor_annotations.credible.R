...
#             annotated = "annotated/AD.meta.annotated.selected.probable.tsv",
#             indir = "paintor_cred/input",
#             outdir = "paintor_cred/input")
...
checkThresholds = function(annot.df) {
  ...
  quantile(annot.df$Roadmap_Enh, na.rm = T, probs = seq(0.0, 1, 0.01))
  quantile(annot.df$DNase, na.rm = T, probs = seq(0.0, 1, 0.01))
...
for (locus in loci.df$locus) {
  fname = paste0(locus, ".in")
  locus.df = readr::read_delim(file.path(opt$indir, fname), delim = " ")
  locus.df = getLocusDF(locus.df)
  locus.annot.df = locus.df %>% select(snp) %>%
    inner_join(annot.binary.df, by = "snp") %>%
  ...
  locus.cred.df = locus.df %>%
    inner_join(annot.binary.df %>% select(snp), by = "snp") %>%
    select(-snp)
  write.table(locus.cred.df, file=file.path(opt$indir, locus), col.names = T, row.names=F, sep=" ", quote=F, na="")
}
