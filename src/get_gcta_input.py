...
freq_dict = {i.split()[0]:[i.split()[3],i.split()[4],i.split()[5]] for i in gzip.open("summary_stats/AD.proxy_exclude_firsts.af.tsv.gz")}
...
for do_chrom in range(1,23)[::-1]:
    ...
    print do_chrom
    #freq_dict = {i.split()[1]:[i.split()[2],i.split()[3],i.split()[4]] for i in gzip.open("summary_stats/ukbb_frequencies/ukbb.british." + do_chrom + ".frq.gz")}
    ...
    write_ma = open("gcta/input/AD.IGAP1_GWAX." + str(do_chrom) + ".ma",'w')
    print >>write_ma, "SNP A1 A2 freq b se p N"
    # A file where we save SNPs that are excluded
    write_excl = open("gcta/input/AD.IGAP1_GWAX.excluded." + str(do_chrom) + ".txt",'w')
    print >>write_excl, "chr SNP A1 A2 freq b se p N exclusion"
    for line in gzip.open(chr_file,'r'):
        vals = line.split()
        if vals[0] != do_chrom:
        ...
        se = vals[12]
        p = vals[13]
        gwas_beta = float(vals[5])
        gwax_beta = float(vals[8])
        het_p = float(vals[16])
        freq_meta = vals[17]
        info = vals[18]
        
        if p == "nan":
            print >>write_excl, do_chrom + " " + snpid + " " + a1 + " " + a2 + " " + freq_meta + " " + beta + " " + se + " " + p + " " + n + " p_is_nan"
            continue
        
        if float(info) < 0.85:
            print >>write_excl, do_chrom + " " + snpid + " " + a1 + " " + a2 + " " + freq_meta + " " + beta + " " + se + " " + p + " " + n + " low_INFO_lt_0.85"
            continue
        
        if not snpid in freq_dict:
            print >>write_excl, do_chrom + " " + snpid + " " + a1 + " " + a2 + " " + freq_meta + " " + beta + " " + se + " " + p + " " + n + " freq_not_found_in_ukbb"
            continue
        
        if het_p < 0.001:
            print >>write_excl, do_chrom + " " + snpid + " " + a1 + " " + a2 + " " + freq_meta + " " + beta + " " + se + " " + p + " " + n + " low_het_p"
            continue
        ...
        if not newSnpID in ukbb_id_dict:
            ...
            if not newSnpID in ukbb_id_dict:
                print >>write_excl, newSnpID + "\tnot in ukbb_id_dict"
                continue
        ...
        if (float(freq) < MIN_FREQ or (1-float(freq)) < MIN_FREQ):
            print >>write_excl, newSnpID + "\tfreq ({0}) below MIN_FREQ ({1})".format(freq, MIN_FREQ)
            continue
        
        print >>write_ma, newSnpID + " " + a1 + " " + a2 + " " + freq + " " + beta + " " + se + " " + p + " " + n
    #os.remove(chr_file)
    write_ma.close()
