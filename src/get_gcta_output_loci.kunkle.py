...
write_hits = open(args.out,'wt')
print("Chr	lead_SNP	lead_pos	SNPs	pos	start	stop	n_snps	lead_freq	lead_beta	lead_p	freq	beta	p", file=write_hits)
for chrom in range(1,23):
    chrom = str(chrom)
    print(chrom)
    gcta_filepath = os.path.join(args.gcta_dir, "AD.Kunkle." + chrom + ".jma.cojo")
    if not os.path.exists(gcta_filepath):
    	gcta_filepath = os.path.join(args.gcta_dir, "AD.Kunkle." + pthreshStr + "." + chrom + ".jma.cojo")
    	if not os.path.exists(gcta_filepath):
        	continue
    # 1. Collapse variants in the same locus
    cojo = [i.split() for i in open(gcta_filepath, 'rt') if i.split()[0] != "Chr" and float(i.split()[7]) < pthresh]
    indep_pos = []
    if len(cojo) > 0:
        indep_pos = [[cojo[0]]]
    ...
    for i in indep_pos:
        ...
        print("\t".join([chrom,lead_snp,lead_pos,snps,poss,str(start),str(stop),str(len(i)),lead_freq,lead_beta,lead_p,freqs,betas,ps]), file=write_hits)
write_hits.close()
