...
for i in open(args.locus_file, 'rt'):
    if i.split()[0] == "Chr":
        continue
    ...
    locus_fname = os.path.join(args.outdir, locus + ".Kunkle")
    
    a1_dict = {j.split()[1]:[j.split()[4],j.split()[5]] for j in open("gcta/input/ukbb_sample." + chrom + ".merged.bim", 'rt') if int(j.split()[3]) >= start and int(j.split()[3]) <= stop}
    ...
    freq_dict = {}
    for j in gzip.open(ukbb_freq_fname, 'rt'):
        line = j.split()
        if len(line) < 2:
            continue
    ...
    # order matters
    finemap_z_file = open(locus_fname + ".z", 'wt')
    print("rsid chromosome position allele1 allele2 maf beta se", file=finemap_z_file)
    
    paintor_locus_file = open(locus_fname + ".paintor", 'wt')
    print("rsid chromosome position allele1 allele2 maf beta se Z", file=paintor_locus_file)
    
    # snplist file for LD file
    snplist_fname = locus_fname + ".snplist"
    write_snplist = open(snplist_fname, 'w')
    write_missing = open(locus_fname + ".excluded_snps", 'wt')
    locus_tmp_file = locus_fname + ".gwas.bgz"
    os.system("tabix {} {}:{}-{} | bgzip > {}".format(args.gwas_file, chrom, start, stop, locus_tmp_file))
    num_snps_written = 0
    for j in gzip.open(locus_tmp_file, 'rt'):
        vals = j.split()
        pos = int(vals[1])
        if pos >= start and pos <= stop:
            ...
            if math.isnan(p):
                print(snp + "\tp is nan", file=write_missing)
                continue
            if not snp in a1_dict:
                # Try the reverse order of alleles
                snp = "{}_{}_{}".format(snpid, a2, a1)
                if not snp in a1_dict:
                    print(snp + "\tnot in a1_dict", file=write_missing)
                    continue
            ...
            if not a1 in a1_dict[snp]:
                print(snp + "\tfails allele match", file=write_missing)
                continue
            ...
            if not snp in freq_dict:
                print(snp + "\tnot in freq_dict", file=write_missing)
                continue
            freq = freq_dict[snp]
            if beta < 0:
                z = z * -1.0
            ...
            if freq < args.min_freq:
                print(snp + "\tSNP has freq <= {}".format(args.min_freq), file=write_missing)
                continue
            if het_p < args.hetp_threshold:
                print(snp + "\thet_p value {} below threshold {}".format(het_p, args.hetp_threshold), file=write_missing)
                continue
            if float(info) < args.min_info:
                print(snp + "\tSNP info ({}) is less than cutoff of {}".format(info, args.min_info), file=write_missing)
                continue
                
            print("{} {} {} {} {} {} {} {}".format(snp, chrom, pos, a1, a2, freq, beta, se), file=finemap_z_file)
            print("{} {} {} {} {} {} {} {} {}".format(snp, chrom, pos, a1, a2, freq, beta, se, z), file=paintor_locus_file)
            print(snp, file=write_snplist)
    ...
    write_missing.close()
    #gc.collect()
    sys.stderr.write("{} SNPs written to .snplist and .z files\n".format(num_snps_written))
...
for i in open(args.locus_file, 'rt'):
    ...
    sys.stderr.write(locus + "\n")
    locus_fname = os.path.join(args.outdir, locus + ".Kunkle")
    write_ld = open(locus_fname + ".temp.ld",'at')
    ld = [i.split() for i in open(locus_fname + ".ld", 'rt')]
    for i_ in range(len(ld)):
        for j_ in range(len(ld)):
            ...
            if i_ == j_:
                ...
                if rsq == "nan":
                    rsq = "0"
            print(rsq + " ", file=write_ld, end="")
        print("", file=write_ld, end="\n")
    write_ld.close()
    os.system("mv " + locus_fname + ".temp.ld " + locus_fname + ".ld")
    #gc.collect()
